...
import { parseQueryState, createMergedQuery, parseQueryStateValue, toQueryStateValue } from 'query-state-core';
...
function useHashQueryStringInterface(_a) {
    var _b = _a === void 0 ? {} : _a, _c = _b.disabled, disabled = _c === void 0 ? false : _c;
...
var useIsoLayoutEffect = isSSR ? useEffect : useLayoutEffect;
...
function useQueryReducer$1(itemName, reducer, initialStateOrInitialArg, initStateFnOrOpts, queryStateOpts) {
    var _a;
...
function useIsValidQueryStateValue(value) {
    var defaultQueryStateValue = useMemo(function () { return toQueryStateValue(value); }, [value]);
...
function useQueryState$1(itemName, initialState, queryStateOpts) {
    ...
    if (typeof initialState === 'function') {
        return useQueryReducer$1(itemName, reducer, undefined, initialState, queryStateOpts);
    }
    return useQueryReducer$1(itemName, reducer, initialState, queryStateOpts);
}
...
function useLocationReducer(itemName, reducer, initialStateOrInitialArg, maybeInitStateFnOrOpts, opts) {
    var locationStateInterface = (opts ||
        ...
        locationStateOptsDefaults$1).locationStateInterface;
    // itemName & defaultValue is not allowed to be changed after init
    var defaultValue = useState(function () {
    ...
    itemName = useState(function () {
        ...
        return "".concat(itemName, "__").concat(suffix);
    })[0];
...
function useLocationState(itemName, initialState, opts) {
    if (opts === void 0) { opts = locationStateOptsDefaults; }
    if (typeof initialState === 'function') {
...
var abortUpdateWillBatch = null;
var useNextRouterQueryStringInterface = function () {
...
var useQueryState = function (key, defaultValue) {
    return useQueryState$1(key, defaultValue, {
...
function useQueryReducer(itemName, reducer, initialState, queryStateOpts) {
    return useQueryReducer$1(itemName, reducer, initialState, __assign({ queryStringInterface: useNextRouterQueryStringInterface() }, queryStateOpts));
}
/**
...
export { getServerSideProps, useLocationState, useQueryReducer, useQueryState };
